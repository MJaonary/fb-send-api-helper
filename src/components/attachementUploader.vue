      ...
      if (this.selectedMode == "send_from_url") {
        ...
        try {
          formData.append(
            "message",
            `{"attachment":{"type":"${
              this.$refs.file_type_input.value
            }", "payload":{"is_reusable":${
              !this.not_reusable ? "true" : "false"
            }}}}`
          );
          formData.append("file", this.file.files[0]);
          this.response = await axios.post(
            `https://graph.facebook.com/v14.0/me/message_attachments/`,
            formData,
            {
              headers: {
          ...
          if (error.name == "TypeError") {
            alert(
              "You must specify one file to upload. Otherwise It will not work."
            );
          } else {
            alert(
              `Error while Uploading file from FILE ${error.response?.data.error.message}`
            );
          }
        }
      }
...
<template>
  <div class="container d-flex flex-column col-lg-8">
    ...
    <div class="border border-primary mt-2 p-2">
      <div v-if="selectedMode == 'send_from_url'">
        <form>
          <div class="form-group">
            <label for="file_url"
              >Enter The url of the file to send here :</label
            >
            <input
              type="email"
              class="form-control"
      ...
      <div v-if="selectedMode == 'send_from_file'" class="border rounded">
        <div class="file-drop-area">
          <span class="choose-file-button btn border-secondary"
            >Choose files</span
          >
        ...
        <div v-if="file?.value" class="border m-2 p-2 rounded">
          {{ file?.value }}
        </div>
      </div>
    </div>
      ...
      <form>
        <div class="form-group">
          <label for="page_access_token"
            >Enter your Page Access Token here :</label
          >
          <input
            type="text"
            class="form-control"
        ...
        <div class="form-check">
          <input
            ...
            id="attachmentReuse"
            v-model="not_reusable"
          />
          <label class="form-check-label" for="attachmentReuse"
            >Not Reusable</label
          >
        </div>
    ...
    <div class="d-flex justify-content-center">
      <div class="btn border m-1" @click="showHistory = !showHistory">
        Show history
      </div>
    </div>
  </div>
